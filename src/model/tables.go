...
type User struct {
	...
	Password_hash string        `gorm:"size:128"`                     //用户密码，加密的，由于 user 使用范围很广，不建议将其 json 标签设为忽略
...
type House struct {
	gorm.Model                    //房屋编号
	UserID          int           //房屋主人的用户编号，与用户进行关联
	AreaID          int           //归属地的区域编号，和地区表进行关联
	Title           string        `gorm:"size:64" `                 //房屋标题
	Address         string        `gorm:"size:512"`                 //地址
	Room_count      int           `gorm:"default:1" `               //房间的数目
	Acreage         int           `gorm:"default:0" json:"acreage"` //房屋总面积
	Price           int           `json:"price"`
	Unit            string        `gorm:"size:32;default:''" json:"unit"`               //房屋单元，eg. X室X厅
...
// 区域信息是需要手动添加到数据库中的
type Area struct {
	ID     int      `json:"aid"`                  //区域编号，eg. 1 2
	Name   string   `gorm:"size:32" json:"aname"` //区域名字，eg. 昌平 海淀
	Houses []*House `json:"houses"`               //区域所有的房屋，与房屋表进行关联
}

// Facility 设施信息 table_name = "facility" 设施信息 需要我们提前手动添加的
type Facility struct {
	ID     int      `json:"fid"`                                      //设施编号
	Name   string   `gorm:"size:32"`                                  //设施名字
	Houses []*House `gorm:"many2many:house_facilities" json:"houses"` //都有哪些房屋有此设施  与房屋表进行关联的
}

// HouseImage 房屋图片 table_name = "house_image"
type HouseImage struct {
	ID      int    `json:"house_image_id"`      //图片id
	Url     string `gorm:"size:256" json:"url"` //图片url，存放房屋的图片
	HouseId int    `json:"house_id"`            //图片所属房屋编号
}

// OrderHouse 订单 table_name = order
type OrderHouse struct {
	gorm.Model            //订单编号
	UserID      int       `json:"user_id"`       //下单的用户编号，与用户表进行关联
	HouseID     int       `json:"house_id"`      //预定的房间编号，与房屋信息进行关联
